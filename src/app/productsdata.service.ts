...
export class ProductsdataService {
  ...
  addToCart(id:string){
    ...
    if(this.cart.has(parseInt(id))){

      let n = this.cart.get(parseInt(id));
      this.cart.set(parseInt(id) , n+1);
      this.total += parseInt(this.products[parseInt(id)-1].product_price)

    }
    else{

      this.cart.set(parseInt(id),1);
      this.total += parseInt(this.products[parseInt(id)-1].product_price)
    }
